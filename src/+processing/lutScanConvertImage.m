function Data = lutScanConvertImage( BFStruct, ImgInfo, Nx, Nz )
    ...
    parameters.NTheta = 0;
    for k=1:length(BFStruct.Lut)
        parameters.NTheta = parameters.NTheta + BFStruct.Lut(k).NLines * BFStruct.Lut(k).NRecon;
    end
    parameters.Nx = Nx ;
    parameters.Nz = Nz ;
    ...
    lutsize = int32(processing.ScanConversionTable( parameters_array, 0 ));
    ...
    [ lutsize_check, LUT_coeff, LUT_X, LUT_Z, LUT_Vois1, LUT_Vois2 ] = ...
