function  BFStruct = lutBfoc(seq,ModeId,ImgInfo,NThreads)
    ...
    NRecon=0;
    for idx=1:length(Lut)
    	NRecon = NRecon+Lut(idx).NRecon ;
    end
    BFStruct.Info.NThreads = NThreads ;
    BFStruct.Lut = Lut ;
    BFStruct.IQ = ones( [ NPixLine NLines*NRecon], 'single' ) + sqrt(-1) * ones( [ NPixLine NLines*NRecon], 'single' );
...
%     ReconAbsc = [ ReconAbsc ReconAbsc ];
    ...
    for idxThread = 1:NThreads
        ...
        [ Status LutSizeBytes Lut(idxThread).data ] = complutRubi( Probe, AcqInfo, ReconInfo, locReconMux, locReconAbsc, locReconDelay, 0 );
        ...
        LutSizeArray(idxThread) = LutSizeBytes ;
    end
end
