function BFStruct = lutFlats( seq, ModeId, ImgInfo, NThreads, varargin )
    ...
    if ~isfield( ImgInfo, 'UseMultiPlaneWave' )
        ImgInfo.UseMultiPlaneWave = 0;
    end
    
    if ~isfield( ImgInfo, 'UseMultiPlaneWave_delays' )
        ImgInfo.UseMultiPlaneWave_delays = [];
    end
...
function [Lut,LutSizeArray,LutInfos] = ComputeLut( seq, ModeId, ImgInfo )
    ...
    for k = 1 : NbSuccessiveAccum : length(TxIds)
        ...
        if ImgInfo.UseMultiPlaneWave
            FlatAngle = double( ImgInfo.FlatAngles(k) * pi / 180 );
        else
        ...
        if ImgInfo.UseMultiPlaneWave
            ReconDelay = double( ImgInfo.UseMultiPlaneWave_delays(k, ReconAbsc + 1) );
        else
