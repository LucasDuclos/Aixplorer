...
if strcmpi(DataFormat, 'RF')
    ...
    time_to_remoteGetShmRFData = 0;
    time_to_remoteGetRFData = 0;
    if strcmp(obj.Server.addr, '127.0.0.1')
        tic_remoteGetShmRFData = tic;
        Status = remoteGetShmRFData(obj.Server, buf, controlSequenceFunction );
        time_to_remoteGetShmRFData = toc(tic_remoteGetShmRFData);
    else
        tic_remoteGetRFData = tic;
        Status = remoteGetRFData(obj.Server, buf, controlSequenceFunction );
        time_to_remoteGetRFData = toc(tic_remoteGetRFData);
    end
...
time_get_data_all = toc( tic_init );


if Debug
    disp( [ 'Time to init : ' num2str(time_to_init) ' s' ] );
    disp( [ 'Time to time_to_remoteGetShmRFData : ' num2str(time_to_remoteGetShmRFData) ' s' ] );
    disp( [ 'Time to time_to_remoteGetRFData : ' num2str(time_to_remoteGetRFData) ' s' ] );
    disp( [ 'Time to getData() : ' num2str(time_get_data_all) ' s' ] );
end
