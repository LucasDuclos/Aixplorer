...
% Compute aperture (initializing constant apertures across firings)
[MinApert, MaxApert] = computeAperture(TxCenter, TxWidth);
...
for k = 1 : length(xApex)
    ...
    % Adaptative apertures
    if length(TxCenter)>1
        [MinApert, MaxApert] = computeAperture(TxCenter(k), TxWidth(k));
        TW = TW.setParam('TxElemts', MinApert:MaxApert);
    end
...
function [MinApert, MaxApert] = computeAperture(TxCenter, TxWidth)
    if length(TxCenter) == 1
        % 1st TX channel
