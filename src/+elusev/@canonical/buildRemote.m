...
try
    ...
    QFilter    = single(obj.getParam('RxBandwidth'));
    Bandwidth  = single(obj.getParam('FIRBandwidth'));
...
for k = 1:length(TxElemts)
    ...
    TW     = TW.setParam( ...
        'repeat', repeat, 'repeat256', repeat256, 'ApodFct', 'none', ...
        'TxElemts', TxElemts(k), 'DutyCycle', DutyCycle, 'TwFreq', TwFreq, 'NbHcycle', 2, ...NbHcycle, ... 
        'Polarity', 1, 'txClock180MHz', txClock180MHz);%, 'UseDualFreq', 1 );
%     TW  = TW.setParam( 'repeat', repeat, 'repeat256', repeat256, 'ApodFct', 'none', ...
%         'TxElemts', TxElemts, 'DutyCycle', DutyCycle, 'TwFreq', TwFreq, 'NbHcycle', NbHcycle, ... 
%         'Polarity', 1, 'txClock180MHz', txClock180MHz );
    % NbHcycle
    if rem(NbHcycle, 2)
