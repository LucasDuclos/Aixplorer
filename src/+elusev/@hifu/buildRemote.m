...
if ( (RxDuration ~= 0) && isempty(find(RxElemts == 0, 1)) )
    ...
    % Control RXFREQ
    txClockFreq = 90 * ( 1 + obj.getParam('txClock180MHz') );
    AuthFreq          = txClockFreq ...
    ...
    RxFreq            = txClockFreq ./ (ADRate * ADFilter);
...
TW     = TW.setParam( ...
    'txClock180MHz', obj.getParam('txClock180MHz'), ...
    'TwFreq', TwFreq, ...
    'TxElemts', TxElemts, ...
    'DutyCycle', DutyCycle );
...
% Initialize TX
TxList = {};
TX     = remote.tx_arbitrary( 'txClock180MHz', obj.getParam('txClock180MHz'), obj.Debug );
...
% force TX
%TxTw = [ 0 22 1023 1 ] ;
%TxTw(1) = TxTw(1,3)*TxTw(1,2)*256/1e6 ;
