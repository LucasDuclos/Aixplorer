...
UseMultiPlaneWave = obj.getParam('UseMultiPlaneWave');
...
TW_NbHcycle = 2;
TW_repeat = NbHcycle / 2 - 1 ;
if rem(NbHcycle, 2)
    NewHcycle = factor(NbHcycle);
    TW_NbHcycle = NewHcycle(1);
    TW_repeat = NbHcycle / NewHcycle(1) - 1 ;
end

TW = TW.setParam( 'NbHcycle',TW_NbHcycle, 'repeat',TW_repeat );
...
if UseMultiPlaneWave
    TW = remote.tw_multi_planewave( ...
        'DutyCycle', DutyCycle, ...
        'TwFreq', TwFreq, ...
        'NbHcycle', NbHcycle, ...
        obj.Debug);
...
for k = 1 : NbFlatAngles

    TX = TX.setParam( 'FlatAngle', FlatAngles(k) );
    if UseMultiPlaneWave
        TX = TX.setParam( 'FlatAngle', 0 ); % to get 0 delays
    end
    ...
    for n = 1:Nb_Tx_Per_Line
        ...
        % TW
        if UseMultiPlaneWave
            TW = TW.setParam( 'FlatAngles', FlatAngles );
            TW = TW.setParam( 'AngleId', k );
        else
            TW = TW.setParam( 'Polarity', TxPolarity(n) );
        end
        
        TW = TW.setParam( 'TxElemts', LocalAperturePattern );
        TwList{end + 1} = TW;
        ...
        EventTxDur = ceil( 1/TwFreq * NbHcycle / 2 + max(TX.getParam('Delays')) ); % bug : TX.getParam('Delays') is always 0
        
        if UseMultiPlaneWave
            TotalDelays = max( TX.computeDelays( min(FlatAngles) * pi/180) ) + max( TX.computeDelays( max(FlatAngles) * pi/180) ) ;
            EventTxDur = ceil( NbFlatAngles * 1/TwFreq * NbHcycle / 2 + TotalDelays ); % TODO : add padding between flats
        end
