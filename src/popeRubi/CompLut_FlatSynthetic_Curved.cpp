...
{
    int NPiezos, NLines, NPixLine, NSlices=0, NPixSlice, NRecon, PlaneWave, BandWidth, TxAlongRx, Folding, ImmediateLut;
...
//     double PiezoWidth, PiezoPitch, XOrigin, LinePitch, SteerAngle,RcvAngle, DemodFrequ, SoundSpeed;
    double PiezoWidth, PiezoPitch, PiezoAngularPitch=0,RadiusOfCurvature, XOrigin, LinePitch, SteerAngle, DemodFrequ, SoundSpeed;
...
//                             0x1120504, 0x1220504, 0x1020504, 0x0020504, 0x0220504, 0x0020100, 0};         // Definition of allowed modes
    int AvailableModes[] = {0x1110308, 0x1210308, 0x1410308, 0x1610308, 0x1010308, 0x0010308, 0x0210308, 0x1210504, 0x1410504, 0x1610504, 0x1010504, 0x0010504, 0x0210504, 0x0010100, 
                            0x1120504, 0x1220504, 0x1420504, 0x1020504, 0x0020504, 0x0220504, 0x0020100, 0};         // Definition of allowed modes
...
//      logon::logon()->debug("lut", "common", "COMPLUT SYNTHETIC FLAT FM ");
    ...
    RadiusOfCurvature = Probe[3] ; // mm unit. Radius of curvature
    if (RadiusOfCurvature > 0) {
        PiezoAngularPitch = PiezoPitch/RadiusOfCurvature ; // radian unit. Piezo pitch.
    }
    ...
    PeakDelay = ReconInfo[7] ;          // �sec unit . Delay between start of transmit pulse, and maximum of transmit power.
...
//     mexPrintf("RcvAngle = %f\n", RcvAngle);
    ...
    if ( (OutputType != 0) && (OutputType != 1) && (OutputType != 2)&& (OutputType != 10)           // Output type must be 0,1, 2, 10, 100,101,102, or 110
            ...
            &&(OutputType != 200) && (OutputType != 201) && (OutputType != 202)&& (OutputType != 210) && (OutputType != 13) && (OutputType != 3))return 14; 
    if ( ((OutputType%100) >= 10) && ((OutputType%100) < 20) && (TxAlongRx >= 10) ) return 23;
    if ((TxAlongRx >=10)&&(PlaneWave==0)) return 24;
    if (((OutputType%100) >= 10) && ((OutputType%100) < 20) && (RadiusOfCurvature > 0) ) return 25;
    if (NLines > NLinesMax)return 26;
...
//         mexPrintf("InterpolationGain1 = %f\n", InterpolationGain);
    ...
    // 24 : Rectangular output without steering forbidden in focalized mode
    // 25 : Rectangular output not implemented for curved array
    // 26 : Too much lines. increase NLinesMax
    // 27 : Too much slices. Increase NSlicesMax
    
...
//         Accum=(Mode & 0x00200000) >> 21;                  // Get Accum parameter
//     }
//     else if ( Mode==0) {
//         ForcedMode = 0;  
...
//     else if ( Mode==-1) {
//         ForcedMode = 0;  
//         Accum=1;                                // An accumulation mode is asked for
//     }
//     else {
...
//     }
    ...
    // Set Bandwidth
    Mode = BandWidth/10;                       
    if (Mode >=10) Mode =Mode/10;                // Case of 200% and 100% modes
    ...
    if (NSlices > NSlicesMax) return 27;
...
//     RecoLutMoinsStart = (int *) &Lut[ProbeSize + AcqInfoSize + ReconInfoSize + InterCoefSize + IQSumSize + GlobalLutSize + AntennaGainSize];
    ...
    int j, m, k, p, Indlut, Arditty, ApodInt, AntennaLeft, IndSample, IndCoef;  
    double SliceLength, ZSlice[NLinesMax*NSlicesMax], XPiezo, ZPiezo, XSlice[NLinesMax*NSlicesMax], XLine, ZLine, IndPiezo, ROrigin, BetaLine, BetaPiezo, SliceDist; 
//     float A, B;
    double Lambda, ReturnDistance, OrPiez, TransmitDistance[NLinesMax*NSlicesMax], XArdi[NLinesMax*NSlicesMax], ZArdi[NLinesMax*NSlicesMax]; //, tanRcvAngle, sinSteerAngle, Const1;
    double TimeOfFlightCenterSlice, TimeOfFlightStartSlice, TimeCycle, cospiezo, sinpiezo, Rcos=0, R2sin2=0, Rsin=0, Sinsteer=0, Cossteer=0 ;
    short SliceStart;
...
//     }
    ...
    ////////////////////////////////////////////////////////
    // Use constants to speed up computation
//     tanRcvAngle=tan(RcvAngle);
//     sinSteerAngle=sin(SteerAngle);
//     Const1=(1-sin(SteerAngle-RcvAngle)*sin(SteerAngle-RcvAngle))/cos(SteerAngle);
    //*********************************************************************************************************
    //*********************************************************************************************************
    // Start Recolut exploration
    //
    // first loop on reconstructed lines to compute slices coordinates, coordinates for apodization, and transmit distances.
    //
    // First compute reference piezo coordinates and angle
    SliceLength = NPixSlice*PitchPix*Lambda ;  // Slice length along line in mm.
    ROrigin=ZOrigin;
    if (RadiusOfCurvature ==0) {                  // Case of linear array
        BetaPiezo=0;
        XPiezo = 0.5*PiezoPitch;
        ZPiezo = 0;
    }
    else {                                      // Case of curved array
        BetaPiezo=PiezoAngularPitch*0.5;
        XPiezo = RadiusOfCurvature*sin(BetaPiezo);
        ZPiezo = RadiusOfCurvature*cos(BetaPiezo);
    }
    // Then loop on lines and slices
    m=0;
    if (TxAlongRx <10) {        // Case of reconstruction along steer angle
        if ((OutputType%100) < 10) {  // case of reconstruction along firing line
            for (j=0; j<NLines; j++) {                      // loop on all lines
                if (RadiusOfCurvature ==0) {                  // Case of linear array
                    BetaLine=0;
                    XLine = (XOrigin + j*LinePitch)*PiezoPitch;
                    ZLine=0;
                }
                else {                                      // Case of curved array
                    BetaLine= (XOrigin + j*LinePitch)*PiezoAngularPitch;
                    XLine= RadiusOfCurvature*sin(BetaLine);
                    ZLine= RadiusOfCurvature*cos(BetaLine);
                }
                for (i=0; i<NSlices; i++) {                 // loop on all slices
                    // first compute coordinates of slice center
                    SliceDist = ROrigin + SliceLength*(i + ((double)(NPixSlice-1))/2/NPixSlice);
                    XSlice[m]=XLine + SliceDist*sin(BetaLine+SteerAngle);
                    ZSlice[m]=ZLine + SliceDist*cos(BetaLine+SteerAngle);
                    // then compute transmit distance
                    TransmitDistance[m]=(ZSlice[m]-ZPiezo)*cos(BetaPiezo+SteerAngle)+ (XSlice[m]-XPiezo)*sin(BetaPiezo+SteerAngle);
                    // Now compute coordinates for arditty coef computation
                    if (TxAlongRx ==0) {
                        XArdi[m]= XSlice[m];
                        ZArdi[m]= ZSlice[m];
                    }
                    else {
                        XArdi[m]= XLine + SliceDist*sin(BetaLine);
                        ZArdi[m]= ZLine + SliceDist*cos(BetaLine);
                    }
                    m+=1;
                }
            }
        }       
        else {                          // case of reconstruction along firing angle, but on normal polar or grid for coherence summation
            for (j=0; j<NLines; j++) {                      // loop on all lines 
                for (i=0; i<NSlices; i++) {                 // loop on all slices
                }
            }
    ...
    else {                      // case of reconstruction along normal to line origin
        for (j=0; j<NLines; j++) {                      // loop on all lines
            if (RadiusOfCurvature ==0) {                  // Case of linear array
                BetaLine=0;
                XLine = (XOrigin + j*LinePitch)*PiezoPitch;
                ...
                XLine= RadiusOfCurvature*sin(BetaLine);
                ZLine= RadiusOfCurvature*cos(BetaLine);
                Sinsteer=sin(SteerAngle);
                Cossteer=cos(SteerAngle);
                Rcos=RadiusOfCurvature*Cossteer;
                Rsin=RadiusOfCurvature*Sinsteer;
                R2sin2=RadiusOfCurvature*RadiusOfCurvature*Sinsteer*Sinsteer;
            }
            for (i=0; i<NSlices; i++) {                 // loop on all slices
                // first compute coordinates of slice center
                SliceDist = ROrigin + SliceLength*(i + ((double)(NPixSlice-1))/2/NPixSlice);
                XSlice[m]=XLine + SliceDist*sin(BetaLine);
                ZSlice[m]=ZLine + SliceDist*cos(BetaLine);
                // then compute transmit distance
                if (RadiusOfCurvature ==0) {                  // Case of linear array
                    TransmitDistance[m]=(ZSlice[m]-ZPiezo)*cos(SteerAngle)+ (XSlice[m]-XPiezo)*sin(SteerAngle);
                }
                else {                                      // Case of curved array
                    // First compute distance to originating piezo
                    OrPiez=sqrt((RadiusOfCurvature+SliceDist)*(RadiusOfCurvature+SliceDist) -R2sin2) - Rcos;
                    //then correct from time of firing between originating piezo and reconstructed line
                    TransmitDistance[m]=OrPiez -Rsin*atan(OrPiez*Sinsteer/(RadiusOfCurvature+OrPiez*Cossteer));
                }
                // Now compute coordinates for arditty coef computation
                if (TxAlongRx ==10) {
                    XArdi[m]= XSlice[m];
    ...
    //*****************************************************************************************************************
    // Now loop on all points and all channels to compute delay times depending on arditty coefficient
    for (k = 0; k<(2-Folding)*MaxChannelsOneSide; k ++) {         //Loop on NChannels on both sides of Region
        
        if ( k < MaxChannelsOneSide ) {                 // Case of piezo on right side of region piezo units
            IndPiezo = k + 0.5;              // Piezo abscissa mm units
            AntennaLeft = 0;                            // to accumulate Channel contribution on right
        }
        else {
            IndPiezo = MaxChannelsOneSide - k - 0.5;     // Case of piezo on left side of region piezo units
        ...
        // Compute Piezo coordinates.
        if (RadiusOfCurvature ==0) {  // case of linear probe
            cospiezo=1;
            sinpiezo=0;
            XPiezo=IndPiezo*PiezoPitch;
            ZPiezo=0;
        }
        else {                          // Case of curved probe
            cospiezo=cos(IndPiezo*PiezoAngularPitch);
            sinpiezo=sin(IndPiezo*PiezoAngularPitch);
            XPiezo=RadiusOfCurvature*sinpiezo;
            ZPiezo=RadiusOfCurvature*cospiezo;
        }
        m=0;
        for (j=0; j<NLines; j++) {                      // loop on all lines
            ...
            for (i=0; i<NSlices; i++) {                      // loop on all slices
                ...
                ReturnDistance = sqrt((XPiezo-XSlice[m])*(XPiezo-XSlice[m]) + (ZPiezo-ZSlice[m])*(ZPiezo-ZSlice[m]));
                Arditty=2048+int(1000*2*(((XPiezo-XArdi[m])*cospiezo - (ZPiezo-ZArdi[m])*sinpiezo)/(ReturnDistance+0.001))*PiezoWidth/Lambda + 0.5);    // Apodisation centered around line origin
                if (Arditty >= 4096) {
                    Arditty=4095;
                }
                if (Arditty < 0) {
...
//                     TransmitDistance = ZSliceCenter*Const1 + XfromBeamCenter*sinSteerAngle;
//                     TimeOfFlightCenterSlice = (TransmitDistance + ReturnDistance)/Lambda;
                    TimeOfFlightCenterSlice = (TransmitDistance[m] + ReturnDistance)*1000*DemodFrequ/AvSoundSpeed[int(TransmitDistance[m] + ReturnDistance+0.5)];
...
//                            mexPrintf("TimeCycle = %f\n", TimeCycle);
  //                          mexPrintf("((NPixSlice-1)*2*PitchPix) = %f\n", ((NPixSlice-1)*2*PitchPix));
    //                        mexPrintf("MaxDelay = %f\n", MaxDelay);
      //                      mexPrintf("(NSamples+FirstSample-8) = %f\n", (NSamples-8));
                    ...
                    // Case of 100% bandWidth
                        ...
                        if ((TimeCycle + ((NPixSlice-1)*2*PitchPix) + MaxDelay)*2 > (NSamples-8)) return 9;  // Image depth is too large for available RF data.
                        if (SliceStart <0) {                    // This is the first Slice contributed to by this channel
...
//     mexPrintf("RecoLutMoinsStart = %d\n", RecoLutMoinsStart[0]);
    ...
    ReconInfoOut[ 7] = PeakDelay;            // �sec unit . Delay between start of transmit pulse, and maximum of transmit power.
