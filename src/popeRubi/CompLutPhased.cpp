...
int CompLutPhased(double *Probe, double *AcqInfo, double *ReconInfo, int *ReconMux, int *ReconAbsc, double * ReconDelay, short *Lut)
{
    ...
    double half_aperture, half_angular_aperture, H, radius_of_curvature, ZOrigin0 ;
...
//     #define FFactorFlat 1.0                     // Coefficient for weighting in flat
    ...
    radius_of_curvature = (-1.0f)*Probe[3] ; // distance between apex and first piezo (left)
    printf("POPE PHASED --------- roc =%f\n",radius_of_curvature);
    half_angular_aperture = asin(PiezoWidth*(NPiezos-1.0f)/2.0f/radius_of_curvature);
    half_aperture = PiezoPitch*NPiezos/2 ;
    H = radius_of_curvature*cos(half_angular_aperture);
    ...
    printf("POPE PHASED --------- ZOrigin = %f\n",ZOrigin);
    printf("POPE PHASED --------- H = %f\n",H);
    printf("POPE PHASED --------- LinePitch = %f\n",LinePitch);
    printf("POPE PHASED --------- SteerAngle = %f\n",SteerAngle);
    ////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if ( (OutputType != 0) && (OutputType != 1) && (OutputType != 2)&& (OutputType != 10)           // Output type must be 0,1, 2, 10, 100,101,102, or 110
            ...
            &&(OutputType != 200) && (OutputType != 201) && (OutputType != 202)&& (OutputType != 210)) return 2214;
...
//     RecoLutMoinsStart = (int *) &Lut[ProbeSize + AcqInfoSize + ReconInfoSize + InterCoefSize + IQSumSize + GlobalLutSize + AntennaGainSize];
    ...
    int j, jrect, k, p, Indlut, Arditty, ApodInt, AntennaLeft, IndSample, IndCoef, DeltaJ, JZero;
    //double SliceThickness, ZSliceCenter, XPiezo, XSlice, XLine, XfromBeamCenter;
    double SliceThickness, ZSliceCenter, XPiezo, ThetaLine ;
//     float A, B;
    ...
    /*if ( ((OutputType%100) >= 10) && ((OutputType%100) < 20) ) {        // Case of rectangular output
        ...
        SliceThickness = NPixSlice*PitchPix*Lambda;         // Depth of slice in mm.
    }*/
...
//     }
    ...
    for (k = 0; k<(2-Folding)*MaxChannelsOneSide; k ++) {         //Loop on NChannels on both sides of Region
        ...
        if ( k < MaxChannelsOneSide ) {                 // Case of piezo on right side of region mm units
            XPiezo = (k + 0.5 + ReconAbsc[0])*PiezoPitch - half_aperture ;           // Piezo abscissa mm units
            AntennaLeft = 0;                            // to accumulate Channel contribution on right
        }
        else {
            XPiezo = (MaxChannelsOneSide - k - 0.5 + ReconAbsc[0])*PiezoPitch - half_aperture ;     // Case of piezo on left side of region mm units
        ...
        for (j=0; j<NLines; j++) {                      // loop on all lines
            ...
            ThetaLine = (SteerAngle + j*LinePitch) ;     // Line origin Abscissa abscissa in radian
            for (i=0; i<NSlices; i++) {                      // loop on all slices
                ZSliceCenter = ZOrigin0 + SliceThickness*(i + ((double)(NPixSlice-1))/2/NPixSlice); // ZOrigin0 : from the apex
                ...
                ZSliceCenter = ZOrigin0 + SliceThickness*(i + ((double)(NPixSlice-1))/2/NPixSlice); // ZOrigin0 : from the apex
                ReturnDistance = sqrt((ZSliceCenter*cos(ThetaLine)-H)*(ZSliceCenter*cos(ThetaLine)-H)+(ZSliceCenter*sin(ThetaLine)-XPiezo)*(ZSliceCenter*sin(ThetaLine)-XPiezo));

                Arditty= 0 ; //2048+int(1000*2*((XPiezo-XSlice + TxAlongRx*(XSlice - XLine))/(ReturnDistance+0.001))*PiezoWidth/Lambda + 0.5);    // Apodisation centered around line origin
                /*if (Arditty >= 4096) {
                    Arditty=4095;
                }
                if (Arditty < 0) {
                ...
                if ((RXApod[Arditty] > 0) || (SliceStart>=0)) {          // Case where the channel contributes to the point
                    TransmitDistance = ZSliceCenter*cos(ThetaLine-SteerAngle) - H/cos(SteerAngle);
//                     TimeOfFlightCenterSlice = (TransmitDistance + ReturnDistance)/Lambda;
                    ...
                    // Case of 100% bandWidth
                        ...
                        // Test to avoidfetching data out of RF file
                        if ((TimeCycle+MinDelay) < 0.25)
                        {
                            printf("POPE PHASED ---------TransmitDistance = %f\n",TransmitDistance );
                            printf("POPE PHASED ---------ReturnDistance = %f\n",ReturnDistance );
                            printf("POPE PHASED ---------PeakDelay*DemodFrequ = %f\n",PeakDelay*DemodFrequ );
                            printf("POPE PHASED ---------PipeOffset = %d\n",PipeOffset );
                            printf("POPE PHASED ---------double(FirstSample)/2 = %f\n",double(FirstSample)/2);

                            return 8;
                        }  // ZOrigin too small for available RF data.
