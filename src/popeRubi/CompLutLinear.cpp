...
int CompLutLinear(double *Probe, double *AcqInfo, double *ReconInfo, int *ReconMux, int *ReconAbsc, double * ReconDelay, short *Lut)
{
    ...
    double PiezoWidth, PiezoPitch, XOrigin, LinePitch, SteerAngle,RcvAngle=0, DemodFrequ, SoundSpeed, MeanAngleSynthetic;
...
//     #define FFactorFlat 1.0                     // Coefficient for weighting in flat
    ...
    // ReconInfo[22]; // arditty_slice treshold
    // ReconInfo[23]; // Sound speed layer
    // ReconInfo[24]; // layer_height
    MeanAngleSynthetic = ReconInfo[25] ; 

	if(TxAlongRx<10) // cas general
    {
        RcvAngle=SteerAngle;
    }    
	if ((TxAlongRx >=10) && (MeanAngleSynthetic!=0.)) // cas plane wave color
    {
        RcvAngle = MeanAngleSynthetic*M_PI/180;
        TxAlongRx = 1;
    }
	if ((TxAlongRx >=10) && (MeanAngleSynthetic==0.)) // cas elasto synthetique
	{
		RcvAngle=0;
    ...
    if ( (OutputType != 0) && (OutputType != 1) && (OutputType != 2)  && (OutputType != 3) &&  (OutputType != 10)&& (OutputType != 13)           // Output type must be 0,1, 2, 10,3, 100,101,102, or 110
            &&(OutputType != 100) && (OutputType != 101) && (OutputType != 102)&& (OutputType != 110)
            &&(OutputType != 200) && (OutputType != 201) && (OutputType != 202)&& (OutputType != 210)) return 14; 
...
//     }
    ...
    // Set Bandwidth
//     mexPrintf ("Lut Mode : %d\n", Mode);
//     mexPrintf ("BandWidth: %d\n", BandWidth);
    Mode = BandWidth/10;
    if (Mode >=10)
...
//     mexPrintf ("Lut Mode 2 : 0x%08X\n", Mode);
    ...
    if (NRecon==1)
	{
		MinDelay = ReconDelay[0]*DemodFrequ;
		MaxDelay = ReconDelay[0]*DemodFrequ;
	}
	else
	{
...
// 	mexPrintf ("Delay: %f\n", Delay);
    ...
    // Case where NPixSlice is not imposed (NPixSlice = 0). Automatic NPixslice choice and automatic mode only
        ...
        if ((8*PitchPix) == int(8*PitchPix)) {
            mexPrintf ("possible multipixel slice: 1\n");
        ...
        if (ModeOk ==0 ) {
            mexPrintf ("possible multipixel slice: 0\n");
    ...
    for (k = 0; k<(2-Folding)*MaxChannelsOneSide; k ++) {         //Loop on NChannels on both sides of Region
        ...
        for (j=0; j<NLines; j++) {                      // loop on all lines
            ...
            for (i=0; i<NSlices; i++) {                      // loop on all slices
                ...
                //Arditty=2048+int(2*1000*sin(-asin(((XPiezo-XSlice + TxAlongRx*(XSlice - XLine))/(ReturnDistance+0.001))*PiezoWidth) - MeanAngleSynthetic*M_PI/180)/Lambda + 0.5);    // Apodisation centered around line origin
                if (Arditty >= 4096) {
                    Arditty=4095;
                }
                if (Arditty < 0) {
...
//                     TimeOfFlightCenterSlice = (TransmitDistance + ReturnDistance)/Lambda;
                    ...
                    // Case of 100% bandWidth
                        ...
                        if ((TimeCycle + ((NPixSlice-1)*2*PitchPix) + MaxDelay)*2.0 > (NSamples-8.0)){
                            mexPrintf("at line = %d (TimeCycle + ((NPixSlice-1)*2*PitchPix) + MaxDelay)*2.0 > (NSamples+FirstSample-8)) \n", __LINE__);
                            mexPrintf("(TimeCycle + ((NPixSlice-1)*2*PitchPix) + MaxDelay)*2.0 = %f\n", (TimeCycle + ((NPixSlice-1)*2*PitchPix) + MaxDelay)*2.0);
                            mexPrintf("((NPixSlice-1)*2*PitchPix) = %f\n", ((NPixSlice-1)*2*PitchPix));
                            mexPrintf("MaxDelay = %f\n", (double)MaxDelay);
                            mexPrintf("NSamples = %d\n", NSamples);
                            mexPrintf("FirstSample = %d\n",FirstSample);
                            mexPrintf("transmit distance = %f\n", TransmitDistance );
                            mexPrintf("ReturnDistance = %f\n", ReturnDistance );
                            mexPrintf("XPiezo = %f\n", XPiezo );
                            mexPrintf("XSlice = %f\n", XSlice );
                            mexPrintf("ZSliceCenter = %f\n", ZSliceCenter );
                            return 9;  // Image depth is too large for available RF data.
                        }
                        if (SliceStart <0) {                    // This is the first Slice contributed to by this channel
...
//     mexPrintf("RecoLutMoinsStart = %d\n", RecoLutMoinsStart[0]);
	...
	ReconInfoOut[25] = MeanAngleSynthetic;
