...
{
    int NPiezos, NLines, NPixLine, NSlices=0, NPixSlice, NRecon, PlaneWave, BandWidth, Folding, ImmediateLut;
    int FirstSample, NSamples, NChannels, MaxChannelsOneSide, PipeOffset, LutSizeBytes, NeededLutSizeBytes;
    double PiezoWidth, PiezoPitch, PiezoAngularPitch,RadiusOfCurvature, XOrigin, LinePitch, SteerAngle, DemodFrequ, SoundSpeed;
    double ZOrigin, ROrigin, PitchPix, BandInterp, FootInterp, PeakDelay, Delay, MaxDelay, MinDelay,RXApodOne, RXApodZero, ka3;
...
//     #define FFactorFlat 1.0                     // Coefficient for weighting in flat
    ...
    ROrigin = ZOrigin + RadiusOfCurvature ; // mm unit. radius of the origin starting at the probe apex (center of curvature)
    ...
    if ( (OutputType != 0) && (OutputType != 1) && (OutputType != 2)           // Output type must be 0,1, 2, 100,101,102,
            &&(OutputType != 100) && (OutputType != 101) && (OutputType != 102)
            &&(OutputType != 200) && (OutputType != 201) && (OutputType != 202)&& (OutputType != 210)) return 3314;


    //if ( ((OutputType%100) >= 10) && ((OutputType%100) < 20) && (TxAlongRx >= 10) ) return 23; // TODO :to be checked for curved probe !
...
// REMOVED (curved)
    ...
    //ProbeSize = 4*4; // 3->4 (roc of curved probe)
    ProbeSize = 3*4; // 3->4 (roc of curved probe)
    AcqInfoSize = 11*4;
    ReconInfoSize = 26*4;
    ReconInfoNext=ReconInfoSize/4;
    //ReconInfoSize += 4 ; // to align InterCoef data (128 bits), new parameter in Probe array,  4+11+22 is not an even number
...
//     RecoLutMoinsStart = (int *) &Lut[ProbeSize + AcqInfoSize + ReconInfoSize + InterCoefSize + IQSumSize + GlobalLutSize + AntennaGainSize];
    ...
    int j, k, p, Indlut, Arditty, ApodInt, AntennaLeft, IndSample, IndCoef;
    double SliceThickness, RSliceCenter, XPiezo, YPiezo, ThetaPiezo, ThetaLine, XSlice, YSlice;
//     float A, B;
    ...
    // these modes are not available for curved probe
    ...
    //}
    //else {
    SliceThickness = NPixSlice*PitchPix*Lambda ; // removed for curved array : *cos(RcvAngle);         // Depth of slice in mm.
    //}
...
//     }
    ...
    for (k = 0; k<(2-Folding)*MaxChannelsOneSide; k ++) {         //Loop on NChannels on both sides of Region
        ...
        ThetaPiezo = (k + 0.5)*PiezoAngularPitch;              // Piezo abscissa radian units
        AntennaLeft = 0;                            // to accumulate Channel contribution on right
        }
        else {
            ThetaPiezo = (MaxChannelsOneSide - k - 0.5)*PiezoAngularPitch;     // Case of piezo on left side of region radian units
        ...
        double XPiezoL = RadiusOfCurvature*sin(ThetaPiezo-PiezoAngularPitch/2.0f);
        double XPiezoR = RadiusOfCurvature*sin(ThetaPiezo+PiezoAngularPitch/2.0f);
        double YPiezoL = RadiusOfCurvature*cos(ThetaPiezo-PiezoAngularPitch/2.0f);
        double YPiezoR = RadiusOfCurvature*cos(ThetaPiezo+PiezoAngularPitch/2.0f);

        XPiezo = RadiusOfCurvature*sin(ThetaPiezo);
        YPiezo = RadiusOfCurvature*cos(ThetaPiezo);

        for (j=0; j<NLines; j++) {                      // loop on all lines
        ThetaLine = (XOrigin + j*LinePitch)*PiezoAngularPitch;     // Line origin Abscissa abscissa in radian
            SliceStart = -1;                                 //Starting line slice to which contributes every channel

        for (i=0; i<NSlices; i++) {                      // loop on all slices

        RSliceCenter = ZOrigin + SliceThickness*(i + ((double)(NPixSlice-1))/2/NPixSlice);

        double threshold = ka3;
        double arditty_slice = RSliceCenter;
        double YSlice_arditty;
        double XSlice_arditty;

        if (RSliceCenter >= threshold)
        {
            arditty_slice = threshold;

        }
        YSlice = RadiusOfCurvature*cos(ThetaLine) + RSliceCenter*cos(ThetaLine+SteerAngle) ;
        XSlice = RadiusOfCurvature*sin(ThetaLine) + RSliceCenter*sin(ThetaLine+SteerAngle) ;

        YSlice_arditty = RadiusOfCurvature*cos(ThetaLine) + arditty_slice*cos(ThetaLine+SteerAngle) ;
        XSlice_arditty = RadiusOfCurvature*sin(ThetaLine) + arditty_slice*sin(ThetaLine+SteerAngle) ;


        double buff = sqrt((XPiezoL-XSlice_arditty)*(XPiezoL-XSlice_arditty)+(YPiezoL-YSlice_arditty)*(YPiezoL-YSlice_arditty)) - sqrt((XPiezoR-XSlice_arditty)*(XPiezoR-XSlice_arditty)+(YPiezoR-YSlice_arditty)*(YPiezoR-YSlice_arditty));

        Arditty=2048+int(2*1000*fabs(buff)/Lambda + 0.5);    // Apodisation centered around line origin
        ...
        double xtan = (XPiezo-XSlice) ;
        double ytan = (YPiezo-YSlice) ;
        xtan *=xtan ;
        ytan *=ytan ;

        ReturnDistance = sqrt( xtan + ytan ); // mm unit.

                if ((RXApod[Arditty] > 0) || (SliceStart>=0)) {          // Case where the channel contributes to the point

        xtan = (XSlice - RadiusOfCurvature*sin(ThetaTX)) ;
        ytan = (YSlice - RadiusOfCurvature*cos(ThetaTX)) ;

        TransmitDistance = xtan*sin(ThetaTX+SteerAngle) + ytan*cos(ThetaTX+SteerAngle) ;
...
//                     TimeOfFlightCenterSlice = (TransmitDistance + ReturnDistance)/Lambda;
                    ...
                    if (BandWidth == 200) {
                        ...
                        if ((TimeCycle + ((NPixSlice-1)*2*PitchPix)+MaxDelay)*4 > (NSamples+FirstSample-8)){
                            printf("line = %d\n", __LINE__);
                            printf("TimeCycle = %f\n", TimeCycle);
                    ...
                    // Case of 100% bandWidth
                        ...
                        if ((TimeCycle + ((NPixSlice-1)*2*PitchPix) + MaxDelay)*2.0 > (NSamples+FirstSample-8.0)){
                            ...
                            printf("NSamples = %d\n", NSamples);
                            printf("FirstSample = %d\n",FirstSample);
                            printf("(NSamples+FirstSample-8)/2.0 = %f\n", (NSamples+FirstSample-8.0)/2.0);
                            return 9;  // Image depth is too large for available RF data.
                        }
                        if (SliceStart <0) {                    // This is the first Slice contributed to by this channel
...
//     printf("RecoLutMoinsStart = %d\n", RecoLutMoinsStart[0]);
    ...
    //ProbeOut[3] = RadiusOfCurvature ; // mm unit. radius of curvature
    // 'AcqInfo'
