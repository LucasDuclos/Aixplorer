...
bool copyBeamformingStruct(const mxArray * pIn, SBeamformParameters * out)
{
    ...
    if(getIntParam(pIn,"nbPiezos",out->m_nbPiezos))
        return false;
    if(getIntParam(pIn,"nbSources",out->m_nbSources))
        return false;
    ...
    if(getIntParam(pIn,"synthAcq",out->m_synthAcq))
        return false;
    if(getIntParam(pIn,"idxTransmitToBeamform",out->m_idxTransmitToBeamform))
        return false;
   ...
   // angles
    ...
    mxArray * pArray;
    pArray = mxGetField( pIn, 0, "xzSource");
    if(pArray)       
    {
        double * pDouble = mxGetPr(pArray);
        for(int i=0; i< out->m_nbSources;i++)
        {
            out->m_xSource[i] = (float)pDouble[2*i];
            out->m_zSource[i] = (float)pDouble[2*i+1];
            mexPrintf("m_xSource[%d] = %g\n",i,out->m_xSource[i]);
            mexPrintf("m_zSource[%d] = %g\n",i,out->m_zSource[i]);
        }
        for(int i=out->m_nbSources; i< NB_MAX_SOURCES;i++)
        {
            out->m_xSource[i] = 0.0f;
            out->m_zSource[i] = 0.0f;
            //mexPrintf("m_xSource[%d] = %g\n",i,out->m_xSource[i]);
            //mexPrintf("m_zSource[%d] = %g\n",i,out->m_zSource[i]);
        }
    }
