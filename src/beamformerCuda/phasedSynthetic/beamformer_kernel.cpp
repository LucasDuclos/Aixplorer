		...
		for (int channel = firstChannel; channel < lastChannel; channel++)
		{
		    ...
		    for (int iSource = firstSource; iSource < lastSource; iSource++)
		    {
		        ...
		        for (int iPixel = 0; iPixel < g_bf_params.m_nbLinesPerRecon; iPixel++)
		        {
                    ...
                    float forwardDelay = sqrt((X-g_bf_params.m_xSource[iSource])*(X-g_bf_params.m_xSource[iSource]) + (Z-g_bf_params.m_zSource[iSource])*(Z-g_bf_params.m_zSource[iSource]))*g_bf_params.m_invSpeedOfSound;
