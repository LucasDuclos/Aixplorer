    ...
    float2 * outImageIQ, float & elapsedTimeMS)
{
    ...
    const int imgSize = inParams->m_nbPixelsPerLine * inParams->m_nbRecon * inParams->m_nbLinesPerRecon ;
    ...
    cudaEventRecord(profileStart, 0);
    
    iq_beamform_kernel<<<grid, block>>>(pRFgpu, bufferSize, pIQOutDatagpu);
    
    cudaEventCreate(&profileEnd);
    cudaEventRecord(profileEnd, 0);
    cudaEventSynchronize(profileEnd);
    ...
    CUDA_SAFE_CALL( cudaMemcpy( outImageIQ, pIQOutDatagpu , imgSize*sizeof(float2), cudaMemcpyDeviceToHost));
