...
% Sequence = usse.usse; Sequence = Sequence.selectHardware
% Sequence = usse.usse; Sequence = Sequence.selectProbe

%% Parameters definition

% System parameters
AixplorerIP    = '192.168.3.228'; % IP address of the Aixplorer device
AixplorerIP    = 'local';
...
ImgInfo.Depth(2)           = 70;  % image depth [mm]
ImgInfo.NbColumnsPerPiezo  = 2;
ImgInfo.NbRowsPerLambda    = 2;
...
UF.TwFreq       = 7.5/(UF.PulseInversion+1) ; % emission frequency [MHz]
UF.NbHcycle     = 4;               % # of half cycles
%UF.FlatAngles   = [ -6:-1 1:6 ]; % [-2 -1 0 1 2 ];               % flat angles [deg]
UF.FlatAngles   = [ -10 -5 5 10 ]; % [-2 -1 0 1 2 ];               % flat angles [deg]
UF.UseMultiPlaneWave = 1;
...
UF.PRF          = 100;            % pulse frequency repetition [Hz], between 2 different flats images (0 for greatest possible)
UF.FrameRateUF  = 100;
UF.FrameRate    = 3;
...
try
    ...
    Acmo = acmo.ultrafast( ...
        ...
        'UseMultiPlaneWave', UF.UseMultiPlaneWave, ...
...
try
    ...
    Sequence = usse.usse( 'TPC',TPC, 'acmo',AcmoList, 'Popup',0, 'DataFormat',DataFormat );
...
UFBfInfo.UseMultiPlaneWave = UF.UseMultiPlaneWave;
if UF.UseMultiPlaneWave
    % get params (simple version)
    A = Sequence.getParam('acmo');
    E = A{1}.getParam('elusev');
    T = E{1}.getParam('tw');
    UFBfInfo.UseMultiPlaneWave_delays = T{1}.TWdelays / 180 ; % us
end

UFBfInfo.FlatAngles = UF.FlatAngles;
for k = 1:NbAcqRx
...
if UF.UseMultiPlaneWave
    ...
    % get canonical RF
    clear RFdata_cano
    NbAngles = length(UF.FlatAngles);
    
    H = hadamard(NbAngles);
    H_inv = inv(H);

    buf_tmp = buf;
    clear Wf_cano RFdata_canos
    for b = 1 : NbAcqRx
        
        for n=1:NbAngles
            RFdata_cano(:,:,n) = 0*buf(b).RFdata(:,:,1);
            Wf_cano(:,:,n) = 0 * Sequence.InfoStruct.tx(1).waveform;
            for n2=1:NbAngles
                RFdata_cano(:,:,n) = squeeze( RFdata_cano(:,:,n) ) + H_inv(n,n2) * buf(b).RFdata(:,:,n2) ;
                
                Wf_cano(:,:,n) = Wf_cano(:,:,n) + H_inv(n,n2) * Sequence.InfoStruct.tx(n2).waveform ;
            end
        end
        
        buf_tmp(b).RFdata = RFdata_cano ;
        buf_out(b) = Sequence.serializeData( buf_tmp(b) );
    end
end
...
for k=1:NbAcqRx
    ...
    if UF.UseMultiPlaneWave
        BFStruct{k} = processing.reconFlat(BFStruct{k}, buf_out(k));
    else
        BFStruct{k} = processing.reconFlat(BFStruct{k}, buf(k));
    end
end

figure(1+UF.UseMultiPlaneWave)
