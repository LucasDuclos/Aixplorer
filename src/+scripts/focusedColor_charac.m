...
do_recon = 1;
NThreads = 4;
IPaddress = '192.168.3.195'; % IP address of Aixplorer
...
ImgInfo.FirstLinePos   = 116 % position of the first line in elt [>=1]
...
RxDelay = ImgInfo.Depth(1) * 1e3 / common.constants.SoundSpeed ;
        ...
        if do_recon
            ...
            for idx=1:ImgInfo.EnsembleLength
                ...
                if system.probe.Radius > 0
                    ...
                    X = ImgInfo.XOrigin  + ( 0:double(ImgInfo.nbTx * ImgInfo.RxLinesPerTx - 1) ) * ( system.probe.Pitch * ImgInfo.txPitch / ImgInfo.RxLinesPerTx ) ;
...
% pin
avg_radius_x = 0.5; % lambda unit
avg_radius_z = 0.5; % lambda unit
pp.offset_dB = -66; % Aixplorer: pin: noise: -10, max:84

% speckle
% avg_radius_x = 4; % lambda unit
% avg_radius_z = 4; % lambda unit
% pp.offset_dB = -63; % Aixplorer: speckle: noise: -10, max:63

% PB equivalent
p.color.nbLines = ImgInfo.RxLinesPerTx ;
p.color.pixelPitch = ImgInfo.PitchPix;
p.color.linePitch = p.color.pixelPitch;

pp.figure = 100;
pp.rx = round( avg_radius_x / p.color.pixelPitch ); % in px
pp.rz = round( avg_radius_z / p.color.linePitch ); % in px
pp.norm_type = 'defined';
pp.norm_value = 100;
pp.X = X;
pp.Z = Z;
...
IQSpectra_normalized_area( IQMatrix, Fe, 1, p, pp, color_rfr );
