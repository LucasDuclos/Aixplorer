function [ Z, noiseFloor, noiseStDev, brightness, mean_roi, mean_roi2 ] = bmode_tgc(ip, voltage, duty_cycle, demodFreq, filterBw, depth, vec_TGC, p, bfg)
    ...
    ImgInfo.RxApod(1)      = 0.6; % RxApodOne < 2
...
%         end
    ...
    vec_roi = 0;
    vec_roi2 = 0;
    zA1 = depth/2-1/20*depth; % mm
    zA2 = depth/2+1/20*depth;
    I=find(Z>=zA1 & Z<zA2);
    for z = I
        for j = 1:size(img_brightness,2)
            vec_roi(ctr) = img_brightness(z,j); 
            vec_roi2(ctr) = img_noisefloor(z,j); 
            ctr = ctr +1; 
        end
    end
    mean_roi = 20*log10(mean(vec_roi));
    mean_roi2 = 20*log10(mean(vec_roi2));
