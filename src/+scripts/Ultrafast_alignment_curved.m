...
%   Revision: 1.00 - Dat e: 2010/03/25
...
% System parameters
AixplorerIP    = '192.168.4.75'; % IP address of the Aixplorer device
...
UF.FlatAngles   = 0;               % flat anSL15_4gles [deg]
...
try
    ...
    while ~strcmpi(pressedKey, 'q');
        ...
        [maxP,absP] = max(abs(processing.hilbert(rfMatrix)),[],1);
        posmax = absP;
        ...
        if length(absP) > 1
            ...
            central_lineenv =  abs(processing.hilbert(central_line));
            v = min(find(central_lineenv == max((central_lineenv))));
            envRfMatrix = abs(processing.hilbert(rfMatrix));
