...
% In order to change the probe used :
% Sequence = usse.usse; Sequence = Sequence.selectProbe

%% Parameters definition

% System parameters
AixplorerIP    = '192.168.3.156'; % IP address of the Aixplorer device
AixplorerIP    = 'local';

DataFormat = 'RF'; % RF: beamforming from Matlab, BF: beamforming in the Aixplorer

FrameRate  = 5; % Hz
repetition = FrameRate*30 ; % repeat sequence [1 inf]
DropFrames = 1;

ImagingVoltage = 70;             % imaging voltage [V]
...
ImgInfo.NbColumnsPerPiezo  = 2.5;
ImgInfo.NbRowsPerLambda    = 2.5;
...
% A0 = [ 4 4.4 ] ; % good cnr vs 4.1
%A0 = [ 4 5.4 ] ;
A0 = [ 2.0000    9.0000    9.5000   10.5000   11.0000 ];
UF.FlatAngles   = [fliplr(-A0) A0]; % [-2 -1 0 1 2 ];               % flat angles [deg]
...
UF.PRF          = 0;1000;            % pulse frequency repetition [Hz], between 2 different flats images (0 for greatest possible)
UF.FrameRateUF  = 0;100;
UF.FrameRate    = FrameRate;3;
...
% try
buf_size = Sequence.RemoteStruct.mode(1).channelSize(1) * system.hardware.NbRxChan / 2 + 2048;
buffer_data = int16( zeros( 1, buf_size, 'int16' ) );
...
for n = 1:repetition
    ...
    tic_get_data = tic;
    for k = 1 : NbAcqRx
        buf{k} = Sequence.getData( 'Realign',0, 'Timeout',4, 'Buffer',buffer_data, 'DataFormat',Sequence.getParam('DataFormat'), 'Debug',Debug  );
    end
...
%     end
    ...
    if n == 1
        ...
        axis equal ;
        axis tight ;
        caxis ( [UF.Threshold UF.DynRangedB + UF.Threshold] );
...
Sequence = usse.usse( 'TPC',TPC, 'acmo',AcmoList, 'Popup',0, 'DataFormat',DataFormat );
Sequence = Sequence.setParam( 'Repeat',1, 'DropFrames',0 );
...
figure(2); imagesc( 20*log10( abs( mean( BFStruct{1}.IQ(:,:,:), 3) ) ) ); colormap gray; caxis([85 160])
